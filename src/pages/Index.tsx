          ...
          {!showSearch && (
            <div className="text-center mb-4">
              {storeConfig?.logo_url ? (
                <img src={storeConfig.logo_url} alt={storeConfig.store_name} className="h-24 w-24 mx-auto rounded-full object-cover mb-3" />
              ) : (
                <div className="h-24 w-24 mx-auto rounded-full bg-primary flex items-center justify-center mb-3">
              ...
              <p className="text-sm text-muted-foreground mt-1">📍 Lima, Cañete</p>
            </div>
          )}
        </div>
      </header>
          ...
          <div className="overflow-x-auto scrollbar-hide">
            <div className="flex gap-3 pb-2 min-w-min">
              ...
              {categories?.map(category => (
                <Button key={category.id} size="sm" variant={selectedCategory === category.id ? "default" : "outline"} onClick={() => setSelectedCategory(category.id)} className="rounded-full shrink-0 text-sm px-3">
                  {category.name}
                </Button>
              ))}
            </div>
